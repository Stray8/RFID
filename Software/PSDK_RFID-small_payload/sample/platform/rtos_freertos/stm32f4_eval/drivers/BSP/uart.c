...
//USART1 handle
UART_HandleTypeDef s_uart1Handle;
#endif
...
UART_HandleTypeDef s_uart4Handle;
#endif
...
{ 	
	while(__HAL_USART_GET_FLAG(&s_uart1Handle, USART_FLAG_TXE) == RESET);  
	s_uart1Handle.Instance->DR = (uint8_t) ch;      
	return ch;
}
#endif 
...
void UART4_IRQHandler(void)                	//串口中断服务程序
{
	...
	if(__HAL_UART_GET_IT_SOURCE(&s_uart4Handle, USART_IT_RXNE) != RESET && __HAL_UART_GET_FLAG(&s_uart4Handle, USART_FLAG_RXNE) != RESET)      	// 接收中断
	{
		uint8_t res;
		//__HAL_UART_CLEAR_FLAG(&s_uart4Handle, USART_FLAG_RXNE);

		res = s_uart4Handle.Instance->DR; 
    	
		// pushRFData((char *)&res, 1);
        uhf_recv_process(res);
    }
} 
