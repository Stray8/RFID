...
extern SemaphoreHandle_t widgetmsg_mutex;
...
#define PSDK_CONSOLE_UART_BAUD          115200

#define PSDK_COMM_WITH_LINUX_UART_NUM     UART_NUM_2
#define PSDK_COMM_WITH_LINUX_UART_BAUD    115200

#define RFID_UART_NUM     UART_NUM_4
...
/* RFID task */
#define USER_RFID_TIM_TASK_STACK_SIZE   	1024
#define USER_RFID_TIM_TASK_PRIORITY     	6

#define USER_RFID_MAIN_TASK_STACK_SIZE   	1024
#define USER_RFID_MAIN_TASK_PRIORITY     	8

#define USER_LED_TASK_STACK_SIZE   	256
#define USER_LED_TASK_PRIORITY     	4
/* Private types -------------------------------------------------------------*/
...
static TaskHandle_t rfid_tim_taskHandle;
static TaskHandle_t rfid_main_taskHandle;
...
void PsdkUser_StartTask(void const *argument)
{
	...
	/* Init RFID UART to MCU */
	UART_Init(RFID_UART_NUM, RFID_UART_BAUD);
	LED_init();
    ...
    if (PsdkProductInfo_SetAlias("PSDK_APPALIAS_DEMO") != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    if (PsdkDataTransmission_Init() != PSDK_RETURN_CODE_OK) {
		PsdkLogger_UserLogError("init data transmission module error.");
		while(1);
	}
	PsdkLogger_UserLogInfo("init data transmission module ok\n");
	
	PsdkLogger_UserLogInfo("create widgetmsg_mutex ... \n");
	widgetmsg_mutex = xSemaphoreCreateMutex();
	while(xSemaphoreTake(widgetmsg_mutex, 2000) == pdTRUE);
	xSemaphoreGive(widgetmsg_mutex);

	PsdkLogger_UserLogInfo("create MsgToMSDK_mutex ... \n");
	MsgToMSDK_mutex = xSemaphoreCreateMutex();
	while(xSemaphoreTake(MsgToMSDK_mutex, 2000) == pdTRUE);
	xSemaphoreGive(MsgToMSDK_mutex);

	PsdkLogger_UserLogInfo("create RfidDev_mutex ... \n");
	RfidDev_mutex = xSemaphoreCreateMutex();
	while(xSemaphoreTake(RfidDev_mutex, 2000) == pdTRUE);
	xSemaphoreGive(RfidDev_mutex);
	
	PsdkLogger_UserLogInfo("create logStr_mutex ... \n");
	logStr_mutex = xSemaphoreCreateMutex();
	while(xSemaphoreTake(logStr_mutex, 2000) == pdTRUE);
    ...
    xTaskCreate((TaskFunction_t) led_task, 
				"led_task", 
				USER_LED_TASK_STACK_SIZE,
                NULL, 
				USER_LED_TASK_PRIORITY, 
				led_taskHandle);
	
	 /* Create RFID TIM task */
    xTaskCreate((TaskFunction_t) uhf_timer_task_demo, 
				"uhf_timer_task_demo", 
				USER_RFID_TIM_TASK_STACK_SIZE,
                NULL, 
				USER_RFID_TIM_TASK_PRIORITY, 
				rfid_tim_taskHandle);


	
	/* Create RFID MAIN task */
	 xTaskCreate((TaskFunction_t) uhf_task_demo, 
				"uhf_task_demo", 
				USER_RFID_MAIN_TASK_STACK_SIZE,
				 NULL, 
				USER_RFID_MAIN_TASK_PRIORITY, 
				rfid_main_taskHandle);
				
	 PsdkLogger_UserLogInfo("RFID task create ok");
...
void led_task(void const *argument){
	...
	while(1){
		...
		switch(rfid_dev.status){
			case UHF_STATUS_IDLE:
				LED_toggle();
				Osal_TaskSleepMs(500);
				break;
			case UHF_STATUS_SCANNING:
				LED_toggle();
				Osal_TaskSleepMs(100);
				break;
			case UHF_STATUS_ERROR:
				LED_ON();
				Osal_TaskSleepMs(1000);
				break;
			case UHF_STATUS_UNINIT:
				LED_toggle();
				Osal_TaskSleepMs(5000);
				break;
			case UHF_STATUS_INITOK:
				LED_toggle();
