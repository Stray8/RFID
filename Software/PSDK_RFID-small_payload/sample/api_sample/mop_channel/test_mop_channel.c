...
REWAIT:
    ...
    while (1) {
        ...
        if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            ...
            PsdkLogger_UserLogDebug("mop channel send data to channel length:%d count:%d", realLen, sendDataCount);
        }
...
REACCEPT:
    ...
    if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        PsdkLogger_UserLogError("mop accept channel error :0x%08llX", returnCode);
        PsdkOsal_TaskSleepMs(TEST_MOP_CHANNEL_RETRY_TIMEMS);
        goto REACCEPT;
    }
    ...
    while (1) {
        ...
        if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            PsdkLogger_UserLogError("mop channel recv data from channel error,stat:0x%08llX", returnCode);
            if (returnCode == PSDK_ERROR_MOP_CHANNEL_MODULE_CODE_CONNECTION_CLOSE) {
                PsdkLogger_UserLogError("mop channel disconnected, now retry accept.");
                s_testMopChannelConnected = false;
                PsdkOsal_TaskSleepMs(TEST_MOP_CHANNEL_RETRY_TIMEMS);
                PsdkMopChannel_Close(s_testMopChannelNormalHandle);
                goto REACCEPT;
            }
        } else {
            PsdkLogger_UserLogDebug("mop channel recv data from channel length:%d", realLen);
            PsdkLogger_UserLogDebug("mop channel recv data from channel content: %d %d %d", recvBuf[0], recvBuf[1],
                                    recvBuf[2]);
        }
    }
}
...
REACCEPT:
        ...
        if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            PsdkLogger_UserLogError("[File-Service] mop accept channel error :0x%08llX", returnCode);
            PsdkOsal_TaskSleepMs(TEST_MOP_CHANNEL_RETRY_TIMEMS);
            goto REACCEPT;
        }
        ...
        returnCode = PsdkOsal_TaskCreate(&s_fileServiceContent[currentClientNum].clientRecvTask,
                                         ...
                                         PSDK_MOP_CHANNEL_TASK_STACK_SIZE, currentClientNum);
        if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
static void *PsdkTest_MopChannelFileServiceSendTask(void *arg)
{
    ...
    uint8_t downloadFileMd5[PSDK_MD5_BUFFER_LEN];
    uint64_t downloadFileTotalSize = 0;
    uint64_t downloadWriteLen = 0;
                ...
                if (downloadWriteLen > 0) {
                    ...
                    if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                        ...
                        PsdkLogger_UserLogDebug(
                            ...
                            (double) (downloadFileTotalSize) * 100 / (double) downloadFileInfo.fileLength);
                    }
                    ...
                    if (fileData.subcmd == PSDK_MOP_CHANNEL_FILE_TRANSFOR_SUBCMD_FILE_DATA_END) {
                        ...
                        if (downloadDurationMs != 0) {
                            downloadRate = (double) downloadFileInfo.fileLength * 1000 / (double) (downloadDurationMs);
                            PsdkLogger_UserLogDebug(
...
static void *PsdkTest_MopChannelFileServiceRecvTask(void *arg)
{
    ...
    uint8_t uploadFileMd5[PSDK_MD5_BUFFER_LEN];
    ...
    while (1) {
        ...
        if (returnCode != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            ...
            if (returnCode == PSDK_ERROR_MOP_CHANNEL_MODULE_CODE_CONNECTION_CLOSE) {
                PsdkLogger_UserLogError("[File-Service] [Client:%d] disconnect from Client now stop task", clientNum);
