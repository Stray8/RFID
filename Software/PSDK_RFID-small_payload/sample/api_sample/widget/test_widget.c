...
char widget_message[PSDK_WIDGET_FLOATING_WINDOW_MSG_MAX_LEN];



//#include "uhf.h"
//#include "uhf_constants.h"
//#include "uhf_config.h"
//#include "utils.h"
//#include "user_var.h"
#include "uhf_demo.h"
...
static void *PsdkTest_WidgetTask(void *arg)
{
    ...
    while (1) {
		...
		psdkStat = PsdkWidgetFloatingWindow_ShowMessage(widget_message);
		if (psdkStat != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
//                           s_widgetTypeNameArray[widgetType], index, value);
	...
	if(index == 3){	// rfid finding switch
		if(value == 1){
	...
	if(index == 7){	// rfid finding threshold set.
		PsdkLogger_UserLogInfo("rfid finding threshold seted as %d", value);
		label_trig_threshold = value;
	}
...
//	if(index == 3)
//		PsdkLogger_UserLogInfo("widgetType:%d || index:%d || value:%d",widgetType, index, *value);
