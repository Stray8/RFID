...
T_PsdkReturnCode PsdkTest_XPortInit(void)
{
    ...
    if (psdkStat != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        PsdkLogger_UserLogError("set roll joint angle limit angle for XPort error: 0x%08llX.", psdkStat);
        return psdkStat;
    }
...
T_PsdkReturnCode PsdkTest_XPortGetSystemState(T_PsdkGimbalSystemState *systemState)
{
    ...
    if (s_isUserXPortInited != true) {
        PsdkLogger_UserLogError("user XPort has not inited.");
        return PSDK_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }
...
static T_PsdkReturnCode ReceiveXPortAttitudeInformation(T_PsdkGimbalAttitudeInformation attitudeInformation)
{
//    PsdkLogger_UserLogInfo("receive XPort attitude information:");
//    PsdkLogger_UserLogInfo("XPort attitude: pitch %d, roll %d, yaw %d.", attitudeInformation.attitude.pitch,
