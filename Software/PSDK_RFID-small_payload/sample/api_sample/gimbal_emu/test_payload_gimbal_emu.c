...
#include "uart.h"
//#include "mycode/GetQuaternion.h"
//#include "led/led.h"
//#include "mycode/flash.h"
...
int pitch_new_value = 0;

void set_pitch(int value);		//设置pitch轴				
...
T_PsdkReturnCode PsdkTest_GimbalInit(void)
{
	...
	PsdkOsal_TaskSleepMs(5000);
	
	set_pitch(1900);	//延时使校准后启动让云台回中
    ...
    switch (rotationMode) {
        ...
        case PSDK_GIMBAL_ROTATION_MODE_ABSOLUTE_ANGLE:
            ...
            if (rotationProperty.absoluteAngleRotation.jointAngleValid) {
                PsdkLogger_UserLogInfo("gimbal joint angles: pitch %d, roll %d, yaw %d.",
...
/******************************************* (dji主) 俯仰轴同步 ***************************************************/								
			pitch_new_value = rotationProperty.absoluteAngleRotation.jointAngle.pitch;				

			if((pitch_new_value>=-900) && (pitch_new_value<=0))
			{
					pitch_value = 1900 + (pitch_new_value/2.25);
...
/******************************************* (dji主) 俯仰轴同步 ***************************************************/								
        ...
        case PSDK_GIMBAL_ROTATION_MODE_SPEED:
            PsdkLogger_UserLogInfo("gimbal rotate speed: pitch %d, roll %d, yaw %d.", rotationValue.pitch,
                                    rotationValue.roll, rotationValue.yaw);
//--------------------------------------------pitch轴控制----------------------------------------------------//
			if(rotationValue.pitch!=0){
				if(rotationValue.pitch>0){//向下
					pitch_sum=pitch_sum+rotationValue.pitch;//进行累加
					int change=0;
					if(pitch_sum>=650){//累加到一定数值，进行角度改变
						change=2;
						pitch_sum=0;
					}
					if(pitch_value<pitch_max){//判断是否超过最下限值
						pitch_value=pitch_value+change;
						if(pitch_value<=pitch_min)//无法继续向上
						{
							pitch_value=pitch_min;
						}
						set_pitch(pitch_value);//设置pitch的值
					}
				}
				else{//向上
					pitch_sum=pitch_sum+rotationValue.pitch;//进行累加
					...
					if(pitch_value>pitch_min){//判断是否超过最上限值
						pitch_value=pitch_value-change;
						if(pitch_value>=pitch_max)
						{
							pitch_value=pitch_max;
						}
						set_pitch(pitch_value);//设置pitch的值
					}
				}
				...
				PsdkLogger_UserLogInfo("pitch停止");
			}
//--------------------------------------------pitch轴控制-------------------------------------------------------//
...
static T_PsdkReturnCode GetAttitudeInformation(T_PsdkGimbalAttitudeInformation *attitudeInformation)
{
    ...
    if (PsdkOsal_MutexLock(s_attitudeMutex) != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
		...
		s_attitudeInformation.attitude.pitch = ((pitch_value-1900)*2.25); 
		s_attitudeInformation.attitude.roll  = 0;
...
//设置最大速度百分比(云台实际最大的转动速度= 默认最大速度 × 最大速度百分比)
static T_PsdkReturnCode SetControllerMaxSpeedPercentage(uint8_t maxSpeedPercentage, E_PsdkGimbalAxis axis)
{
...
static T_PsdkReturnCode Reset(E_PsdkGimbalResetMode mode)
{
	...
	uint8_t resetbuf[7];
	PsdkLogger_UserLogInfo("mode:%d",mode);
	if(mode==3)//回中
	{	
		set_pitch(1500);
		PsdkLogger_UserLogInfo("重启3 回中");
	}
	if(mode==12)//垂直
	{	
		set_pitch(1100);
		PsdkLogger_UserLogInfo("重启12 垂直");
	}
	if(mode==1)//yaw回中
	{	
		PsdkLogger_UserLogInfo("重启11 yaw回中");
	}
	if(mode==11)//回中或垂直
	{
		set_pitch(1100);
		PsdkLogger_UserLogInfo("重启11 垂直");
	}
    PsdkLogger_UserLogInfo("重启 reset gimbal: %d.", mode);
...
void set_pitch(int value){
	uint8_t resetbuf[7];
	PsdkLogger_UserLogInfo("pitch:%d",value);
	resetbuf[0]=0xFA;
	resetbuf[1]=0x02;
	resetbuf[2]=0x0A;//帧头
	resetbuf[5]= 0x33; 
	resetbuf[6]= 0x34; 
	if(value<pitch_min){
		resetbuf[3]=(uint8_t)pitch_min;
		resetbuf[4]=(uint8_t)(pitch_min>>8);
		UART_Write(UART_NUM_1,resetbuf,7);
		pitch_value=pitch_min;
		PsdkLogger_UserLogInfo("pitch max");
		return;
	}
	if(value>pitch_max){
		resetbuf[3]=(uint8_t)pitch_max;
		resetbuf[4]=(uint8_t)(pitch_max>>8);
		UART_Write(UART_NUM_1,resetbuf,7);
		pitch_value=pitch_max;
		PsdkLogger_UserLogInfo("pitch min");
		return;		
	}
	else{
		resetbuf[3]=(uint8_t)value;//低数据位
		resetbuf[4]=(uint8_t)(value>>8);//高数据位
		UART_Write(UART_NUM_1,resetbuf,7);
		pitch_value=value;	
		return;			
	}
