...
T_PsdkReturnCode PsdkTest_PayloadCollaborationInit(void)
{
	...
	PsdkLogger_UserLogInfo("payload collaboration module init ok");
	...
	PsdkLogger_UserLogInfo("payload collaboration task create ok");
	...
	// psdkStat = PsdkPayloadCollaboration_GetCameraTypeOfPayload(aircraftBaseInfo.payloadMountPosition, &cameraType);
	// if (psdkStat != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
	// 	PsdkLogger_UserLogDebug("gimbal read err!");
	// }
	// PsdkLogger_UserLogInfo("camera type of payload mounted on NO.%d gimbal connector is %d.",
	// 					aircraftBaseInfo.payloadMountPosition, cameraType);
